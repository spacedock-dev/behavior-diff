...
out=$(cd "$plainrun" && PATH="$stub:$PATH" "$runner" --file CLAUDE.md --task t \
      --before-file missing.md 2>&1); code=$?
set -e
...
set +e
out=$(cd "$plainrun" && PATH="$stub:$PATH" "$runner" --file CLAUDE.md --task t 2>&1); code=$?
set -e
...
set +e
out=$(cd "$repo" && PATH="$stub:$PATH" "$runner" --file CLAUDE.md --task t 2>&1); code=$?
set -e
...
out=$(cd "$big" && PATH="$stub:$PATH" BEHAVIOR_DIFF_COPY_CAP_KB=16 "$runner" \
      --file CLAUDE.md --task t --before-file orig.md 2>&1); code=$?
set -e
...
set +e
out=$(cd "$plainrun" && PATH="$stub:$PATH" "$runner" --file CLAUDE.md --task t \
      --before-file orig-ABSENT 2>&1); code=$?
set -e
